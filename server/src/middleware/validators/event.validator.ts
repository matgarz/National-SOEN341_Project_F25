...
export const createEventValidation = [
    ...
    body('ticketType')
        .optional()
        .isIn(['FREE', 'PAID']).withMessage('Ticket type must be FREE or PAID'),
    body('ticketPrice')
        .optional()
        .custom((value, { req }) => {
            if (req.body.ticketType === 'PAID') {
                if (!value || parseFloat(value) <= 0) {
                    throw new Error('Ticket price is needed and has to be greater than 0 for paid events');
                }
            }
        return true; }),
    body('category')
    ...
    body('organizerId')
        .optional()
        .isInt({ min: 1 }).withMessage('Invalid organizer ID'),
    ];