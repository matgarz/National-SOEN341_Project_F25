...
import { Check, X, Eye, Users, Calendar, AlertTriangle, TrendingUp } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const handleOrganizerAction = (requestId: string, action: 'approve' | 'reject') => {
  ...
  const pendingOrganizersCount = organizerRequests.filter(r => r.status === 'pending').length;
  const flaggedEventsCount = pendingEvents.filter(e => e.flagged && e.status === 'pending').length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Organizer Requests</CardTitle>
      ...
      {flaggedEventsCount > 0 && (
        <Alert>
          ...
          <AlertDescription>
            {flaggedEventsCount} event{flaggedEventsCount > 1 ? 's' : ''} flagged for review. 
                        ...
                        {request.status === 'pending' ? (
                          <div className="flex space-x-2">
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => handleOrganizerAction(request.id, 'approve')}
                            >
                              <Check className="h-4 w-4 mr-1" />
                              Approve
                            </Button>
